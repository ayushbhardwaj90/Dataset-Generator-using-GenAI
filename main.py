...
def generate_dataset(
    ...
    try:
        # Handle custom domain with free-form prompt
        if request.domain == "Custom" and request.custom_prompt:
            data = generator.generate_custom_data(request.custom_prompt, request.rows, request.constraints)
            domain_name = "Custom"
        else:
            data = generator.generate_sample_data(request.domain, request.rows, request.constraints)
...
def export_csv(
    ...
    # For GET requests, passing complex objects like constraints is tricky.
    # A more robust solution might involve a POST endpoint for exports with a request body.
    # For now, we'll keep it simple and assume constraints are not passed via GET for exports.
    current_user: User = Depends(get_current_user)
):
    try:
        # Note: Constraints from GET are not easily passed for exports.
        # This endpoint will currently generate data without constraints for exports.
        # If constraints are critical for export, consider changing this to a POST endpoint
        # or encoding constraints in a single query parameter.
        if domain == "Custom" and custom_prompt:
            data = generator.generate_custom_data(custom_prompt, rows, constraints=None) # No constraints for GET export
        else:
            data = generator.generate_sample_data(domain, rows, constraints=None) # No constraints for GET export
